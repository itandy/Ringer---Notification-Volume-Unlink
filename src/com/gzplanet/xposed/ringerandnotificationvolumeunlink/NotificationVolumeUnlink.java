...
public class NotificationVolumeUnlink implements IXposedHookZygoteInit, IXposedHookLoadPackage {
    ...
    final static String CLASSNAME_NOTIFICATIONVOLUMEPREFERENCECONTROLLER = "com.android.settings.notification.NotificationVolumePreferenceController";
    final static String CLASSNAME_VOLUMEDIALOGIMPL = "com.android.systemui.volume.VolumeDialogImpl";
    final static String CLASSNAME_VOLUMEROW_O = "com.android.systemui.volume.VolumeDialogImpl.VolumeRow";
    ...
    public void handleLoadPackage(LoadPackageParam lpparam) throws Throwable {
        ...
        if (lpparam.packageName.equals(PKGNAME_SETTINGS)) {
            if (Build.VERSION.SDK_INT >= VERSION_CODES_O) {
                try {
                    final Class<?> classNotifVolumePrefController = XposedHelpers.findClass(CLASSNAME_NOTIFICATIONVOLUMEPREFERENCECONTROLLER, lpparam.classLoader);

                    findAndHookMethod(classNotifVolumePrefController, "isAvailable", new XC_MethodReplacement() {
                        @Override
                        protected Object replaceHookedMethod(MethodHookParam param) throws Throwable {
                            return true;
                        }
                    ...
                    XposedBridge.log("NotificationVolumePreferenceController not found");
                }
            ...
            if (Build.VERSION.SDK_INT >= VERSION_CODES_O) {
                ...
                try {
                    XposedHelpers.findClass(CLASSNAME_VOLUMEDIALOGIMPL, lpparam.classLoader);
                    XposedBridge.log("Class systemui.volume.VolumeDialogImpl found");

                    final Class<?> classVolumeRow = XposedHelpers.findClass(CLASSNAME_VOLUMEROW_O, lpparam.classLoader);

                    XposedHelpers.findAndHookMethod(CLASSNAME_VOLUMEDIALOGIMPL, lpparam.classLoader, "initDialog",
                            new XC_MethodHook() {
                                @Override
                                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
//                                    XposedBridge.log("VolumeDialogImpl:initDialog");
                    ...
                    XposedHelpers.findAndHookMethod(CLASSNAME_VOLUMEDIALOGIMPL, lpparam.classLoader, "updateVolumeRowH", classVolumeRow,
                            new XC_MethodHook() {
                                @Override
                                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                                    //XposedBridge.log("VolumeDialog:updateVolumeRowH");
                                    String notifLabel = "Notification volume";
                                    String ringLabel = "Ring volume";
                    ...
                    XposedBridge.log("Class systemui.volume.VolumeDialogImpl not found");
                } catch (NoSuchMethodError e1) {
                    XposedBridge.log("Method systemui.volume.VolumeDialogImpl.updateVolumeRowH not found");
                }
